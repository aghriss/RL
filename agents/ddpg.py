    ...
    def train(self):
        ...
        while(self.done<self.train_steps):
            ...
            while to_log <self.update_double:
                ...
                # Compute loss
                loss = self.Q.total_loss(current_q,target_q.detach()) # loss = self.Q.total_loss(current_q, target_q)
                
                # Optimize the model
                self.Q.optimizer.zero_grad()
                loss.backward()#error =  target_q-current_q,current_q.backward(-1.0 * error.clamp(-1, 1))
                self.Q.optimize()
